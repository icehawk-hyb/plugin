...
func (bc *BaseClient) CheckTxExpire(txs []*types.Transaction, height int64, blocktime int64) (transactions []*types.Transaction) {
	...
	debugPrintTxHash(txlist.GetTxs(), false)
...
func isExpire(txs []*types.Transaction, height int64, blocktime int64) bool {
	for _, tx := range txs {
		if height > 0 && blocktime > 0 && tx.IsExpire(height, blocktime) {
			log.Info("isExpire", "height", height, "blocktime", blocktime, "hash", common.ToHex(tx.Hash()), "Expire", tx.Expire)
			return true
		}
	}
	return false
}
func debugPrintTxHash(txs []*types.Transaction, startOrEnd bool) {
	StartOrEnd := "start"
	if !startOrEnd {
		StartOrEnd = "end"
	}
	log.Info("CheckTxExpire:debugPrintTxHash------------", "StartOrEnd", StartOrEnd)
	for index, tx := range txs {
		log.Info("debugPrintTxHash", "index", index, "hash", common.ToHex(tx.Hash()))
	}
}
